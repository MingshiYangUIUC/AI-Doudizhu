...
def initialize_difficulty(iPlayer, Model, nhistory, difficulty):
    ...
    while True and i < 1000:
        ...
        while Turn < iPlayer+1: # game loop
            ...
            output = Model(hinput,player_ids).flatten()
...
def gamewithplayer(iPlayer, Model, temperature, pause=0.5, nhistory=6, automatic=False, difficulty=None, showall=False): # Player is 0, 1, 2 for L, D, U
    if difficulty is None:
        Init_states = init_game_3card() # [Lstate, Dstate, Ustate]
    else:
        Init_states = initialize_difficulty(iPlayer, Model, nhistory, difficulty)
...
if __name__ == '__main__':
    ...
    v_M = 'H15-V5_0000500000'

    N_history = int(v_M[1:3])
    N_feature = 5
    Model_Unified = Network_V3_Unified(N_history+N_feature)
    Model_Unified.load_state_dict(torch.load(os.path.join(wd,'models',f'Unified_{v_M}.pt')))
    Model_Unified.eval()
    ...
    with torch.no_grad():
        Turn, Qs, Log = gamewithplayer(player,Model_Unified,temperature,pause,N_history, automatic, difficulty, showall)
...
e:
conda activate rl-0
python E:\\Documents\\ddz\\pvc_unified.py -h
'''
