...
from model import *
from base_funcs_V2 import state2str, init_game_3card
...
def simEpisode_notrain(Initstates, Models, temperature, verbose=0):
    ...
    while True: # game loop
        ...
        if hasattr(Models[Turn%3], '__len__') and len(Models[Turn%3]) == 2:

        #if len(Models[Turn%3]) == 2:
            ...
            action,Q = get_action_serial_V1_1_0(Turn,Models[Turn%3],Initstates,unavail,lastmove,Forcemove,history,temperature)
...
def simulate_match_wrapper(args): # dual match
    ...
    if len(models[p1])==3: # 3 player 3 model
        playersG1 = [models[p1][0],models[p2],models[p2]]
        playersG2 = [models[p2],models[p1][1],models[p1][2]]

    if len(models[p2])==3: # 3 player 3 model
        playersG1 = [models[p1],models[p2][1],models[p2][2]]
        playersG2 = [models[p2][0],models[p1],models[p1]]

    match_result = simEpisode_notrain([i.clone().detach() for i in Deck], playersG1, 0.0, 0)
    match_result = int(match_result % 3 == 0)
    print(p1, p2, match_result)
    pair.append((p1, p2, match_result))

    match_result = simEpisode_notrain([i.clone().detach() for i in Deck], playersG2, 0.0, 0)
    match_result = int(match_result % 3 == 0)
...
if __name__ == '__main__':
    ...
    names = ['H15-V2_1.2-Contester_0035000000','H15-V0_5.0-Contester_0001000000']
    ...
    LM.load_state_dict(torch.load(os.path.join(wd,'models',f'LM_{names[1]}.pt')))
    DM.load_state_dict(torch.load(os.path.join(wd,'models',f'DM_{names[1]}.pt')))
    UM.load_state_dict(torch.load(os.path.join(wd,'models',f'UM_{names[1]}.pt')))
