...
# Create a lazy import for torch
...
# call model only if eval is needed
    ...
    try:
        ...
        data_dir = os.path.join(wd, 'models')
    ...
    SLM = torch.jit.load(os.path.join(wd,'data','SLM_best_scripted.pt'))
    QV = torch.jit.load(os.path.join(wd,'data','QV_best_scripted.pt'))
    '''
