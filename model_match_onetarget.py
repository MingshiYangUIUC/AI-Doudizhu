...
if __name__ == '__main__':
    ...
    players = [str(i).zfill(10) for i in range(30000000,40000000+1,1000000)]
    versions = ['H15-V4'] # only one version

    target = 'H15-V4_0035000000'

    models = []
    nplayer = len(players)*len(versions) + 1
    fullplayers = []
    for v in versions:
        fullplayers += players
    ...
    if 'V4' in target:
        ...
        LM, DM, UM = Network(N_history+4),Network(N_history+4),Network(N_history+4)
    LM.load_state_dict(torch.load(os.path.join(wd,'models',f'LM_{target}.pt')))
    DM.load_state_dict(torch.load(os.path.join(wd,'models',f'DM_{target}.pt')))
    UM.load_state_dict(torch.load(os.path.join(wd,'models',f'UM_{target}.pt')))
    LM.eval()
    DM.eval()
    UM.eval()
    models.append(
    ...
    fullplayers.append(target[-10:])

    ax_player = list(range(len(models)))

    # Create an Elo system instance
    elo_system = EloSystem(num_players=len(models))
    print(len(models))

    outfile = os.path.join(wd,'data',f'elo_FT_{"".join(versions)}_{target}_{fullplayers[0]}-{fullplayers[-2]}.txt')
    outfile2 = os.path.join(wd,'data',f'wingame_FT_{"".join(versions)}_{target}_{fullplayers[0]}-{fullplayers[-2]}.txt')
    ...
    tasks = []
    for _ in range(nsim):
        m1, m2 = random.sample(ax_player[:-1], k=1)[0], ax_player[-1]
        #print(m1,m2)
    ...
    plt.savefig(os.path.join(wd,'data',f'elo_FT_{"".join(versions)}_{target}_{fullplayers[0]}-{fullplayers[-2]}.png'))
    ...
    plt.plot([str(int(i)//10000) for i in fullplayers[:-1]],(wingame/ngames)[:-1])
    plt.axhline((wingame/ngames)[-1],color='red',alpha=0.5)
    #for i,v in enumerate(versions):
    #    plt.plot(fullplayers[i*len(players):(i+1)*len(players)],(wingame/ngames)[i*len(players):(i+1)*len(players)])
    plt.savefig(os.path.join(wd,'data',f'wingame_FT_{"".join(versions)}_{target}_{fullplayers[0]}-{fullplayers[-2]}.png'))
    #plt.show()