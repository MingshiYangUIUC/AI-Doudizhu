...
def all_actions(mystate,forcemove=0):
    #mystate = str2state('JJQQKKAA22998877')
    # opinfo is (type, rank)
    ...
    for i in range(LenS):
        for j in range(i+1):
            if i != j or Nelems[i] >= 2:#mystate[:,c2r_base[s1]].sum() >= 2:
                s1,s2 = sin[i], sin[j]
...
def simEpisode_serial_softmax(Models, temperature, selfplay_device, Nhistory=6):
    ...
    while True: # game loop
        ...
        if len(acts) > 1:
            # generate inputs
            hinput = torch.concat([torch.concat([Bigstate,str2state(a[0]).unsqueeze(0)]).unsqueeze(0) for a in acts])
            hinput = torch.concat([hinput for i in range(100)])
            # get q values
            output = model(hinput.to(selfplay_device)).to('cpu').flatten()[:int(len(hinput)/100)]
...
def simEpisode_batchpool_softmax(Models, temperature, selfplay_device, Nhistory=6, ngame=20, ntask=100):
    ...
    Init_states = [init_game() for _ in range(ngame)] # [Lstate, Dstate, Ustate]
    ...
    while True in Active: # game loop
        ...
        for iin, _ in enumerate(Index[Active]):
            ...
            Tidx = Turn[_]%3
            player, model = Init_states[_][Tidx], Models[Tidx]
            ...
            # get actions
            acts = avail_actions(lastmove[_][0],lastmove[_][1],player,Forcemove[_])

            Bigstate = torch.cat([player.unsqueeze(0),str2state(unavail[_]).unsqueeze(0),CC.unsqueeze(0),history[_]])
